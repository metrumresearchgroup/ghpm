...
get_pull_request_commits <- function(org, repo, number, .cc = FALSE, .api_url = api_url()){
	data <- sanitize_respone(graphql_query("pullrequests/pull_request_commits.graphql", org = org, repo = repo, number = number, .api_url = .api_url))$repository$pullRequest$commits$nodes
