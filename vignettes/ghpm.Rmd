...
```{r}
get_repo_issues("rstudio", "shiny") %>% filter(milestone == "0.9.0", state == "CLOSED")
```

or say I wanted to only get issues that a certain user created for a specific milestone
```{r}
get_repo_issues("rstudio", "shiny") %>% filter(creator == "HarlanH", milestone == "0.9.0")
```
...
```{r}
HarlanH_issues <- get_repo_issues("rstudio", "shiny") %>% filter(creator == "HarlanH", milestone == "0.9.0")
