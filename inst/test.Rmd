...
```{r}
data <- graphql_query("projects.graphql", org = "metrumresearchgroup", repo = "pkgr")$repository$projects$nodes

temp <- data[[1]]$columns$nodes[[4]]$cards$nodes

reduce(temp, function(.acc2, .cv2){
				if(!is.null(.cv2$content) && length(.cv2$content) > 0){
					.acc2 <- .acc2 %>% bind_rows(tibble("Number" = .cv2$content$number, "Title" = .cv2$content$title))
				}
				return(.acc2)
			}, .init = tibble("Number" = NA, "Title" = NA))

lapply(data, function(x){
	return(reduce(x$columns$nodes, function(.acc, .cv){
		column = .cv$name
		if(length(.cv$cards$nodes) > 0){
		...
		return(.acc)
	}, .init = tibble("Column" = NA, "Number" = NA, "Title" = NA)))
})
```
